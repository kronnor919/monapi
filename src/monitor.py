...
async def test_endpoint(
    client: httpx.AsyncClient, domain: str, endpoint_setting: EndpointSettings
) -> TestResult:
    try:
...
async def test_all(test_config: MonitorSettings) -> list[TestResult]:
    async with httpx.AsyncClient() as client:
        tasks = [
            test_endpoint(client, domain.url, endpoint)
